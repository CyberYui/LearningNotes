...
* 安装版 MySQL 安装可视化 , 其卸载也十分简单 , 首先打开 MySQL Installer , 还是之前提到的 Remove 操作

* 点选全部软件 , 然后执行 Execute 操作 , MySQL Installer 会挨个卸载软件 , 十分丝滑

* 全部卸载完成后会看到以下界面 , MySQL Installer 会提示是否卸载其本身 , 这里我们不选择确定 , 我们可以留着它 , 点击 Finish 完成 , 会看到只有一个 MySQL Installer 壳子的界面 , 接下来的操作就不需要我多说明了

  ![MySQL_Install_msi_23](MySQL_Install_msi_23.png) 

* 通过这个 MySQL Installer 的壳子我们可以去安装我们想要的所有 MySQL 产品 , 并且还能控制它们的路径 , 岂不美哉
...
* 如果你之前用过 MySQL , 那么你一定知道有一个 <kbd>mysql.ini</kbd> 文件需要进行设置 , 但是 zip 版数据库可能没有这个文件 , 不用担心 , 自己创建一个就可以了
  ...
  * 至此 , 一次完整的解压版 MySQL 下载 , 安装 和 删除 就完成了
