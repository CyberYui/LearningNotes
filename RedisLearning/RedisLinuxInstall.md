...
* 至此 , 笔者的一个服务器环境就搭建好了 , 这台阿里云服务器里面包括的工具有 , **MySQL** , **Java** , **Tomcat** , **Nginx** 以及 **Redis** 
* 其中 **MySQL** 数据库使用的端口为 **3306**
* **Tomcat** 使用的端口为 **8080**
* **Nginx** 使用和监听的端口为 **80**
* **Redis** 启动的端口为 **6379**
* 这四个端口基本均为默认端口 , 如果有需要可以自行修改
* 笔者搭建这样的一个服务器环境的原因如下
* 一般笔者之前常用的 **Spring MVC** 项目可以放在 **Tomcat** 中 , 对不同项目配给不同端口 , 这个配置可以在 **Tomcat** 的 **server.xml** 文件中配置 , 然后通过 **Nginx** 去隐藏 **Spring MVC** 项目的端口号即可
* 多个项目无论是 **Spring MVC** 或者是 **SpringBoot** 项目 , 都在使用 **MySQL** 数据库 , 而一些对性能要求较高的 **SpringBoot** 前后端分离项目可能会需要使用到 **Redis** 数据库来提高访问速度 , 这些就是后话了